...
``wxWidgets_CXX_FLAGS``
  ...
  Windows. Essentially the output of ``wx-config.bat --cxxflags``.
...
``wxWidgets_CONFIG_OPTIONS``
  ...
  the wx-config.bat utility.  For example, to use the base toolkit found on the
...
# Basically, the intent here is to mimic what wx-config.bat would do by
# default (i.e., `wx-config.bat --libs`).
#
# Possible solution:
...
if(wxWidgets_FIND_STYLE STREQUAL "unix")
    ...
    #-----------------------------------------------------------------
    #
    # Set the default values based on "wx-config.bat --selected-config".
    #
    ...
    #
    # Set wxWidgets_SELECT_OPTIONS to wx-config.bat options for selecting
    ...
    #
    # Look for wx-config.bat -- this can be set in the environment,
    ...
    if(wxWidgets_CONFIG_EXECUTABLE)
      ...
      # get defaults based on "wx-config.bat --selected-config"
      ...
      # run the wx-config.bat program to get cxxflags
      execute_process(
        COMMAND sh "${wxWidgets_CONFIG_EXECUTABLE}"
      ...
      # run the wx-config.bat program to get the libs
      # - NOTE: wx-config.bat doesn't verify that the libs requested exist
    ...
    # When using wx-config.bat in MSYS, the include paths are UNIX style paths which may or may
    ...
    # Check that all libraries are present, as wx-config.bat does not check it
    set(_wx_lib_missing "")
...
# FIXME: set wxWidgets_<comp>_FOUND for wx-config.bat branch
